...
- `modules/desktops/` — pick **one** per host (`gnome.nix`, `plasma.nix`, `dwm/`).
  `desktops/displaymanagers/` holds greeters: pick **at most one**, and only on a host
  whose desktop doesn't already ship one. `gnome.nix` brings gdm and `plasma.nix` brings
  sddm, so a greeter is for bare-WM hosts like dwm.
...
4. **Edit `hosts/<hostname>/default.nix`**:
   ...
   - Adjust the `imports` list — swap the desktop (`modules/desktops/gnome.nix`, `plasma.nix`, `dwm`), a greeter from `modules/desktops/displaymanagers/` if the desktop doesn't bring one, and app sets (`modules/apps/*.nix`) to whatever the new host needs
